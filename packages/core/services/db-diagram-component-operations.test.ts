...
// Component creation stores a thumbnail, exactly as snapshot preservation does.
// The recording store is asked what it holds instead of a directory being
// stat-ed — same assertion, no disk (see diagram-thumbnail-store.ts).
let thumbnails: ReturnType<typeof DiagramThumbnailStore.recording>;

beforeAll(async () => {
  const result = await createTestDb();
  testDb = result.testDb;
  thumbnails = DiagramThumbnailStore.recording();
  testLayer = DiagramComponentOperationsService.Default.pipe(
    Layer.provide(Layer.succeed(DrizzleService, testDb as never)),
    Layer.provide(thumbnails.layer)
  );
});

beforeEach(async () => {
  await truncateAllTables(testDb);
  thumbnails.written.clear();
});
...
describe("createComponent", () => {
  ...
  it.effect("stores the thumbnail before the row exists", () =>
    Effect.gen(function* () {
      const created = yield* create("With a thumbnail");
      // A row must never reference a missing thumbnail.
      expect(thumbnails.written.has(`_components/${created.id}`)).toBe(true);
    }).pipe(Effect.provide(testLayer))
  );
...
describe("deleteComponent", () => {
  ...
  it.effect("removes the thumbnail, after the row", () =>
    Effect.gen(function* () {
      const ops = yield* DiagramComponentOperationsService;
      const created = yield* create("Doomed");
      yield* ops.deleteComponent(created.id);
