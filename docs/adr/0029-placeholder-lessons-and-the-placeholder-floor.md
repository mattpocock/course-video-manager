...
- **Nothing fails any more; everything is visible.** `IncompleteVideosError` is retired. A hard gap decides a status and appears in a list the author reads before pressing the button. The one release-stopping failure left is an invalid Lesson role combo, because roles are then ambiguous and there is no honest node to emit.

- **ADR 0019's no-null rule survives intact.** The two existing Lesson members are untouched: every field on a shipping Video stays required and non-nullable. What changes in ADR 0019 is its consequence, not its reasoning. `body` and Clips are now guaranteed by the classifier, because they are two of the three hard gaps; `description` is guaranteed one step further out, by the course-view lint gate, which refuses a Publish outright — a stricter gate than the throw it replaces, because it refuses before any byte is uploaded.
...
- `buildCourseJson`'s error channel loses `IncompleteVideosError` and keeps `InvalidLessonRoleComboError`. `collectPublishBlockers` still enumerates `incompleteVideos`, but only over shipping Lessons — in practice a missing `description`, the one gap a shipping Video can still have.
