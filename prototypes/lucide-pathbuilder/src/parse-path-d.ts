/**
 * PROTOTYPE — throwaway. SVG path `d` parser.
 *
 * Produces a flat list of absolute segments in the vocabulary tldraw's
 * PathBuilder actually speaks: M / L / C / A / Z.
 *
 * Everything else in the SVG path grammar is folded in on the way through:
 *   - relative commands   -> absolutised
 *   - H / V               -> L
 *   - S (smooth cubic)    -> C (reflect previous control point)
 *   - Q (quadratic)       -> C (exact degree elevation)
 *   - T (smooth quadratic)-> C (reflect, then elevate)
 *
 * Arc flags are read as single characters, so SVGO-style "glued" flags
 * (`a5 5 0 015 5`) parse correctly even though lucide 0.525.0 never emits them.
 */
...
/** Quadratic (P0, Q, P2) -> cubic. Exact: degree elevation, not an approximation. */
  ...
  while (!sc.atEnd()) {
    ...
    switch (cmd.toUpperCase()) {
      ...
      case "S": {
        ...
        // control point about the current point — or the current point itself
      ...
      case "T": {
        const qx = lastQuadCp ? 2 * x - lastQuadCp.x : x;
        const qy = lastQuadCp ? 2 * y - lastQuadCp.y : y;
        const nx = sc.num() + ox;
        const ny = sc.num() + oy;
        out.push(quadToCubic(x, y, qx, qy, nx, ny));
        x = nx;
