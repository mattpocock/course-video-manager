...
OUTPUT (one pretty JSON object)
  ...
  incompleteVideos[]    Shipping Videos missing a required field, and narrowed
                        the same way. A Video with no Clips or no Body is a hard
                        gap instead, so its Lesson does not ship and it is not
                        named here (ADR 0029).
