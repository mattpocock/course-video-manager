...
beforeAll(async () => {
  ...
  // A bulletPanel is authored content-and-all, so every case that asks for one
  // has to pass its bullets: the content rule is checked before the Clip Zoom
  // guard, and these cases are about the guard. One file serves them all — no
  // case here varies the bullets.
  bulletsPath = join(mkdtempSync(join(tmpdir(), "cvm-transform-")), "b.json");
  writeFileSync(
    bulletsPath,
    JSON.stringify([{ icon: "target", text: "Name the problem", revealAt: 0 }]),
    "utf8"
  );
});
...
const addOverlay = (
  ...
  run([
    ...
    "--title",
    "Hydration",
    // Each kind requires its OWN content: a Definition Card's definition, a
    // Bullet Panel's bullets. Passing the other kind's is refused.
