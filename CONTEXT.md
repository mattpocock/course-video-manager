...
**Export Version Key**:
A version marker (`EXPORT_VERSION`) that, when bumped, invalidates every **Export Hash** and so forces every **Video** to be exported again.
...
**Footage**:
A raw source video FILE on disk (an OBS capture, a screen recording) before it is cut into **Clips** — the input to editing, not a product of it. UNLIKE EVERY OTHER NOUN IN THE DOMAIN MODEL, Footage has no database row: its identity is its filesystem path, and its whole-file **Transcription** is cached beside it, keyed by a content hash so replacing or re-recording the file re-transcribes rather than serving stale words — the same "the filesystem **is** the state" convention as a **Video File**. Transcribed whole-file and speaker-agnostic: the words and their timings are kept, and who said them is not modelled. Managed with `cvm footage` (list / transcribe / transcript), all LOCAL-ONLY (it lives on the author's disk); `cvm clip add` reads the cached transcript to give a new **Clip** its text. A Clip's `videoFilename` points at the Footage it was cut from.
...
**Video Warning**:
A derived, non-blocking authoring problem surfaced on a **Video** in the UI. Computed live from the video's clips and chapters — never stored. Each warning has a stable kind (e.g. `missingChapters`). Generalizes the existing per-clip "danger" signal (text similarity) to the video level so course-tree views can flag videos at a glance. The two kinds the **Autofill** owns (`missingChapters`, `missingDescription`) are computed exactly as before but are no longer _shown_ on the authoring surfaces — the course view, the **Section Workbench** and the video editor — because they are no longer the author's work; they remain fully blocking inside **Publish Readiness**. Every other kind is untouched and keeps nagging. One consequence, deliberate: the course view's warning count reads lower than the publish page's blocker count (ADR 0024).
...
**Exported Video**:
A rendered `.mp4` file on disk, addressed by its **Export Hash**, whose duration this machine has vouched for. A file at the address is not enough: a renderer that stops without complaining has not said the file is whole, so an export is measured against the duration its **Clips** ask for, and one that falls more than a second short is a truncation and is refused rather than kept. The verdict is recorded beside the file, together with the **Byte Hash**, so the file is measured once and every later **Publish** reads that answer instead of deriving it again.
...
The persistent store of rendered **Overlay** content — one transparent render per distinct piece of content, whatever its **Overlay Kind**, addressed by that content and nothing else. The address covers exactly what the Kind draws, plus the **Overlay Renderer Version**: for a **Definition Card**, its `title`, `description` and duration; for a **Bullet Panel**, its `title`, duration, every **Bullet**'s `icon`/`text`/`revealAt` in display order, and both **Animation Toggles** (they cut the panel's own animation, so they change the frames). A Definition Card's payload names no Kind at all, exactly as the **Export Hash** omits the default Kind — that omission is what let a second Kind arrive without re-rendering a single card already on disk. It is a second, smaller cache sitting under the **Export Hash**'s whole-export one, and the split is the point: a Video is re-exported whenever any **Clip** of it moves, and re-rendering every Overlay on each of those exports is what the cache exists to avoid. Because the address is the content and nothing else, two Overlays carrying the same content share one render — anywhere in the same course, in any Video. Renders are scoped per course, so a course's renders can be swept up on their own, at the price of rendering shared content once per course. Everything in it is derivable from the database, so deleting the whole cache costs only render time.
_Avoid_: Overlay cache (it holds renders, not **Overlays**), Definition Card cache, Render directory

**Overlay Renderer Version**:
A version marker (`OVERLAY_RENDERER_VERSION`) that, when bumped, invalidates every render in the **Overlay Render Cache** and so forces each **Overlay**'s content to be drawn again. The sibling of the **Export Version Key**, and deliberately not the same marker: a change to how content is drawn should re-render the content and leave every exported video alone, and a change to how a video is encoded should re-export the videos and leave every cached render alone. Adding a new **Overlay Kind** does not bump it — a Kind that did not exist has no cached renders to invalidate, and its addresses are disjoint from every other Kind's.
...
**Silence Length**:
A per-Recording-Session setting (`short` or `long`) for how long a silence must last to end a clip: `short` (default) cuts on brief mid-sentence pauses, `long` only on extended ones. Locked at recording start, and applied symmetrically to the live speech detector and to the cut made afterwards, so the two can never disagree.
...
**Pause**:
A clip-level marker (`none`/`long`) that inserts a held pause after a **Clip** in the edit — `long` holds briefly, `none` is an ordinary clip. Toggled per clip, shown as an ellipsis ("…"). An enum, not a boolean, so it can gain more lengths later. Distinct from **Silence Length** (the recording cut threshold).
_Avoid_: Beat (former `beatType`), Silence Length, Gap, Hold

**Clip Zoom**:
A clip-level marker (`none`/`subtle`) that renders a **Clip** slightly larger than frame — cropped centrally across, biased above centre down the frame — so that a run of face-only camera clips has some visual change across its cuts. Legal only where the Clip's recorded `scene` is a camera scene (`Camera` or `TikTok Face`); every other scene, and a Clip with no recorded scene at all, is refused. An enum, not a boolean, so it can gain more levels later — how much larger is deliberately not encoded in the value names, so retuning the shot is not a migration. ONE framing, expressed as fractions of the frame rather than pixels, drives both the editor's preview and the export, which is what makes the preview honest about what the **Publish** will ship. Cropping happens before the frame is normalised, so a source larger than the output frame spends surplus resolution rather than upscaling. Part of the **Export Hash**, emitted only when not `none`. Set per clip in the editor or with `cvm clip update --zoom`. Superseded by **Transform**, which generalizes this same fractional-rect approach to an animated move that can span across Clips; retained here until that migration ships.
...
`Overlay` and `Definition Card` are implemented as domain data: an agent authors them with `cvm overlay add|list|get|update|delete`, and there is no editor UI. They reach a finished course video by being composited onto the landscape export, which turns each Overlay's Clip-relative anchor into an offset on the flattened Video timeline. Every Overlay of a Video is composited in ONE pass, never a pass per Overlay, and a Video with no Overlays is not composited at all — so such a Video's bytes are what they always were. The pass runs on the joined timeline rather than per Clip, because a **Transform** has to interpolate across a Clip boundary and a per-Clip step cannot express that. A **Transform** lands in that same pass, gated to the same window as the graphic it accompanies, so the footage moves under the graphic in one encode rather than two. That shared window is the whole of the sync contract: an Overlay's content is rendered exactly as long as the Overlay itself, so the graphic arrives on the frame the camera starts moving and leaves on the frame it stops. The pass re-encodes a file the export has already written, so it takes its picture settings from the export's own — a course video carrying an Overlay must not come out with different characteristics from one that does not. The vertical Shorts pipeline composites its own full-length subtitle track by a separate route, untouched by any of this. `Transform` now exists in one narrow form only — the fixed, **Overlay Kind**-derived preset described under that term. `Overlay Template` and `Transition` are still **not yet implemented**: they remain a design in progress, worked out ahead of any code as the intended replacement for today's `Effect Clip` and `Clip Zoom`, so treat those two terms as proposed until this note names them differently.
...
**Transform**:
An **Overlay**'s pan/zoom move on the underlying footage — an ordered sequence of framing keyframes (fractions of frame, following **Clip Zoom**'s convention, not pixels), interpolated in order across the owning **Overlay**'s duration. Independent of whatever content the Overlay carries, if any: a Transform can move the frame with nothing on top, or ride alongside a Card or Diagram to make room for it. The move is made on the footage itself across the Overlay's span, never by passing the footage through the content renderer, which draws an Overlay's own graphic and nothing else. Supersedes **Clip Zoom**, which expressed only a single static crop and could not span across Clips — and, until that supersession actually happens, may not coexist with it: creating an Overlay whose window lands on a Clip with a Clip Zoom is REFUSED, because the two crops compound rather than compose.

What ships today is deliberately the narrowest version: not an authored keyframe sequence but a fixed pair of framings looked up from the **Overlay Kind** — none at all for a Kind that moves no camera and, for a **Bullet Panel**, centred to right-shifted and slightly zoomed, so the left of frame clears for the panel. It eases in and symmetrically back out, unless the Overlay's **Animation Toggles** collapse that end to a cut. The ease is ONE speed for the whole moment: the panel slides in as the presenter's face moves right, because those are one move seen twice, and a panel that has arrived while the camera is still travelling is the one result nobody wants. Both the framings and that speed are being tuned by eye against real renders. The editor's own player moves the footage under the graphic on the same playhead, so what an author sees while editing is what the **Publish** will ship — the same twinning that makes **Clip Zoom**'s preview honest.
...
**Skills Changelog**:
A published AI Hero entity bundling an article and a Kit newsletter draft for one **Video**. Publishes immediately, and creates a Kit newsletter draft as it does — drafts only, never sends. The newsletter is required; article and newsletter are authored on one page. Public at `https://www.aihero.dev/skills/<slug>`, with a footer linking back.
...
**Remote Box**:
Any machine that is not the author's, running `cvm` against the deployed API with an **API Token** and no database connection string. It reaches every domain noun `cvm` exposes — Course, Course Version, Section, Lesson, Video, Clip, Beat, Pitch, Deliverable and search — so it can find work, read and write a **Script** and a **Body**, author and reorder **Beats**, draft **Pitches**, restructure the **Draft Version** and maintain the **Deliverables Calendar**. It can never do anything that needs the author's machine — the finished videos directory, the Video Files directory, the video encoder, OBS.
_Avoid_: Server, Agent machine

**Local-only Command**:
A `cvm` command that needs the author's **machine** rather than the domain data, and so can never run on a **Remote Box**: `cvm file` (the Video Files directory), `cvm course readiness` (the finished videos directory, from which exportedness is derived) and `cvm course publish` (both, plus the video encoder). Each refuses at the front of the command, before any work, naming what it would have needed — a refusal an agent stops on, where a filesystem error is one it retries. Refusing before any work is also what stops a **Course** being left half-changed. The author's machine declares itself with `CVM_LOCAL_MACHINE`; a box that says nothing is treated as remote.
_Avoid_: Offline command, Machine command, Disabled command

**Schema Version**:
The number of database migrations a checkout was built against, stated by `cvm` on every request and compared by the deployed API against its own. Any difference is refused outright, naming both numbers and telling the caller to pull — an out-of-date box cannot write against a schema it does not understand. Migrations are applied by deploying that API and by nothing else, and are additive-only, so a `cvm` already in flight when one lands keeps working.
