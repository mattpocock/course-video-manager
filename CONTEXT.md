...
**Animatic**:
The playback of a **Video**'s ordered **Clip Mockups** — the film term for a timed cut of the storyboard with scratch audio, made before the shoot. Each Clip Mockup holds on screen for exactly as long as its line takes to say, with a fixed 0.08 s gap between them (the same buffer the Clip cutter uses). THERE IS NO ANIMATIC RECORD: the word names the playback of those rows and nothing else. Nothing is stored under this name, and nothing is addressed by it. The author watches it at `/videos/:id/animatic`, ONE PAGE OF THE VIDEO like its editor and its Post tabs — the same sidebar, the same breadcrumb and the same PREVIOUS/NEXT, which walk the Lesson's Videos Animatic to Animatic — and reads the POSITION of the current Clip Mockup in a corner. That position is how a note is given back to the agent — "number 14 is too dense" — because there is no note field on a Clip Mockup. Purpose: to see the Lesson from the student's seat before a filming day, when its problems are still cheap to fix. It plays at TWO TIMES SPEED unless the author says otherwise, from a speed control in the player bar; the speed he picks is remembered in the browser and used for the next Animatic, because the speed is a habit of the watcher and not a property of the Video. The run time in the corner is always the length of the filmed Lesson, never how long this sitting takes. A Video with no Clip Mockups still has the page: it says how they are authored rather than showing a blank frame, because the way in is offered for every Landscape Video. Once the author is happy, the agent writes the Video's **Script** from the Clip Mockup lines.
