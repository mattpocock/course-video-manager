...
**Bullet**:
One line of a **Bullet Panel**: an `icon`, its `text`, and its own `revealAt`. The icon is a lucide name from the vendored, append-only icon table — any name in it is allowed, and one that is not in it is refused when the Overlay is written, not discovered in a render. `revealAt` is SECONDS AFTER THE OVERLAY'S OWN START, which is the whole reason it is authored per bullet: an agent working from a transcript derives it as `wordStartTime - overlayAt` and the bullet appears exactly as the words are spoken. Bullets must be submitted in STRICTLY ascending `revealAt` order matching their display order, so a list can never reveal out of the order it is read in and no two Bullets arrive in the same wave — a tie is refused, because a staggered reveal is the whole point of authoring a time per Bullet. Each Bullet also needs room to finish appearing before the panel starts leaving: 0.35s to ease in, plus the 0.35s the panel spends easing out, so the last Bullet of a 5s panel may be revealed at 4.3s. `disableExitAnimation` gives the second 0.35s back — a cut exit holds the panel to the window's very last frame — so the same Bullet may then be revealed at 4.65s. Each Bullet eases in ALONE at its own moment; the panel LEAVES as one, never staggered, so a four-bullet exit is as quick as a one-bullet exit.
...
What ships today is deliberately the narrowest version: not an authored keyframe sequence but a fixed pair of framings looked up from the **Overlay Kind** (`packages/core/features/videos/overlay-transform.ts`), `null` for a Kind that moves no camera and, for `bulletPanel`, centred (scale 1.0 at origin 0.5/0.5) to right-shifted (about scale 1.3 at origin 0.62/0.4) so the left of frame clears for the panel. It eases in over ~0.35s and symmetrically back out on the same `cubic-bezier(0.25, 0.1, 0.25, 1)` the Subtitle overlay already uses, unless the Overlay's `disableEnterAnimation`/`disableExitAnimation` collapse that end to a cut. Those endpoint numbers are a ballpark, expected to be tuned against a real render; retuning them is a one-line edit in that one file, exactly as **Clip Zoom**'s 115% is. That same pair of framings is also formatted into a CSS `transform` for the editor's own player, so the footage moves under the graphic on the playhead in the editor exactly as it will in the export — the same twinning that makes **Clip Zoom**'s preview honest, and asserted the same way: the emitted `crop` and the emitted CSS are evaluated against one another in pixels, at moments through the ease.
