...
**Effect Clip**:
A special clip for non-speech content (white noise, transitions) manually inserted into the timeline, marked as one by carrying the white-noise `scene` — there is no column for it. Nothing is said over it, so it never has a **Transcript Word** and no **Transcription** would give it one: a re-transcribe pass leaves Effect Clips alone rather than spending a Whisper call on noise and overwriting their label with whatever came back, and one is not counted as a Clip whose word timing is missing. Superseded by **Transition**, which anchors to a boundary between two Clips instead of occupying a fake Clip slot of its own; retained here until that migration ships.
...
**Transcript Word**:
One spoken word of a **Clip**, with the `start`/`end` seconds it was said at — Whisper's word-level timing, kept rather than discarded. Offsets are CLIP-RELATIVE (`0` is the Clip's own start), never offsets into the source **Footage** file and never positions on the finished Video's timeline, so they survive the Footage being re-recorded or moved. A one-to-many child of the Clip (cascade-deleted with it, like **Clip Web Link**), written whole-Clip-at-a-time by a **Transcription** or by slicing the Footage sidecar on `cvm clip add`; ordered by `start`, never edited one at a time. A Clip transcribed before Transcript Words existed simply has none — an ordinary state (`cvm clip words` prints nothing), cleared by re-transcribing, never backfilled by guesswork; an **Effect Clip** has none for good, having no speech to time. A Transcription writes a Clip's words BEFORE the `text`/`transcribedAt` that says it is transcribed, and each Clip of a batch stands alone: one Clip whose audio cannot be read costs only itself, never the rest of the Video its timing. Purpose: addressing a precise spoken moment inside a Clip, which the whole-Clip **Transcript** text cannot express. Retiming its Clip (`cvm clip update --start/--end`) shifts every one of its words by the same delta, in the same transaction — see **Retiming Cascade**.
