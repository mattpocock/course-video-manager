...
**Animatic**:
The playback of a **Video**'s ordered **Clip Mockups** — the film term for a timed cut of the storyboard with scratch audio, made before the shoot. Each Clip Mockup holds on screen for exactly as long as its line takes to say, with a fixed 0.08 s gap between them (the same buffer the Clip cutter uses). THERE IS NO ANIMATIC RECORD: the word names the playback of those rows and nothing else. Nothing is stored under this name, and nothing is addressed by it. The author watches it full screen at `/videos/:id/animatic`, with no editor chrome in the way, and reads the POSITION of the current Clip Mockup in a corner. That position is how a note is given back to the agent — "number 14 is too dense" — because there is no note field on a Clip Mockup. Purpose: to see the Lesson from the student's seat before a filming day, when its problems are still cheap to fix. Once the author is happy, the agent writes the Video's **Script** from the Clip Mockup lines.
