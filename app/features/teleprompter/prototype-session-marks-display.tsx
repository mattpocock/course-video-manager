 ...
 *
 * One dot per clip in the current recording session, oldest first.
 *
 ...
 * gap in a rhythm is easier, on camera, than counting anything.
 *
...
const PER_ROW = 3;
/**
 * A cap only so a long take can't march off the bottom of the panel. The newest
 * are kept, so the tail of the take — the part you're still in — wins the
 * space.
 */
...
export function SessionMarksDisplay(props: { marks: ClipMarks }) {
  ...
  const overflow = Math.max(0, props.marks.length - MAX_DOTS);
  const shown = props.marks.slice(-MAX_DOTS);
