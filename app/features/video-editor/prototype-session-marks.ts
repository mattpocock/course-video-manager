...
export function usePrototypeSessionMarks(
  items: TimelineItem[],
  ...
  return useMemo(() => {
    ...
    for (const item of items) {
      ...
      if (item.type === "optimistically-added") {
        ...
        if (item.sessionId !== currentSessionId) continue;
        marks.push({
          // Paired clips inherit their optimistic clip's insertion order, so
          // this stays comparable with the pending ones above and a mark fills
          // in where it already sat rather than jumping position.
